...
void loop() {
  ...
  if (Serial.available()) {
    ...
    //Serial.println("]");
    if (strcmp(SerialData, "stop") == 0) { // 간단한 명령어 넣어서 동작 시키기
...
float angleCalculator(float newpositionX, float newpositionY) { // x, y 값 읽어서 계산값 넣어서 각도로 바꾸기
...
float distanceCalculator(float newpositionX, float newpositionY) { // 총 움직인 양 계산식
  return sqrt(pow(newpositionX, 2) + pow(newpositionY, 2));
}

/************************322 - 342 참고*****************************/  
float calculator(float newposition1, float newposition2) { // x, y, 각도 계산하는 식
